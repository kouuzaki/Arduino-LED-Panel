...
bool initEthernet() {
  ...
  // Jika tidak terdetect, tunggu sebentar dan coba lagi (DC jack kadang butuh
  // waktu ekstra)
  ...
  case EthernetNoHardware:
    ...
    Serial.println(
        "Check: 1) Shield mounted? 2) Pin 53 as OUTPUT? 3) SPI pins free?");
...
void checkLanConnection() {
  ...
  if (isConnected && !lanWasConnected) {
    ...
    // Let maintain run but don't spam; controller will try to re-init when link
    // returns
    Ethernet.maintain();
  }
...
void setup() {
  ...
  // W5100 chip needs time after power-on to stabilize (voltage regulator,
  // oscillator, etc) DC jack 9V 2A: Arduino boots VERY fast, but W5100 needs
  // significant stabilization time USB Type-A: Has natural delay from USB
  // enumeration (~500-1000ms) This extended delay ensures W5100 internal
  // circuitry is fully ready before SPI communication Especially important for
  // cheap/unstable DC adapters that have slow voltage rise time
  ...
  Serial.println(
      "W5100 Power-Up Delay: 3000ms (3 sec - DC jack 9V max compatibility)");
  ...
  if (FileStorage::begin()) {
    ...
    if (FileStorage::loadDeviceConfig(doc)) {
      ...
      if (doc["subnet_mask"].is<String>())
        subnet.fromString(doc["subnet_mask"].as<String>());
      if (doc["dns_primary"].is<String>())
